...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'legal.vilkar' })
  return withPageSeo(
    { title: t('metaTitle'), description: t('metaDescription') },
    { locale, pathSegments: ['vilkar'], indexable: true, keywords: ['vilkår', 'Hjelpi'] },
  )
}

export default async function VilkarPage() {
  const t = await getTranslations('legal.vilkar')
