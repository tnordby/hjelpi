...
export default async function DashboardKundeOverviewPage() {
  ...
  const buyerBookings = await fetchBuyerBookings(supabase, ctx.profileId)
  const threads = await fetchMessageThreads(supabase, ctx.profileId)
  const unreadTotal = threads.reduce((s, x) => s + x.unreadCount, 0)
  const buyerSpend = await fetchBuyerSpendSummary(supabase, ctx.profileId)
  const pendingBuyer = buyerBookings.filter((b) => b.status === 'pending').length
