...
export default async function DashboardLayout({ children }: { children: ReactNode }) {
    ...
    return (
      <>
        ...
        <div className="mx-auto max-w-2xl px-6 py-28 text-center text-on-surface-variant">
          Min side er ikke tilgjengelig uten Supabase-konfigurasjon.
        </div>
        <Footer />
      </>
    )
  ...
  if (!ctx) {
    // Session exists but no usable profile (or DB error). Do not send to /logg-inn — that page
    // redirects authenticated users back to /min-side → infinite redirects.
    const locale = await getLocale()
  ...
  return (
    <>
      <Navbar />
      <DashboardLayoutShell dbActiveMode={ctx.activeMode}>
        {children}
